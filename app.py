...
def login_validation():
    ...
    curs.execute("""SELECT * FROM `users` WHERE `email` LIKE '{}' AND `password` LIKE '{}'""".format(email,password))
    users = curs.fetchall()
    if len(users)>0:
...
def add_user():
    ...
    curs.execute("""INSERT INTO  users(name,email,password) VALUES('{}','{}','{}')""".format(name,email,password))
    conn.commit()
    curs.execute("""SELECT * FROM `users` WHERE `email` LIKE '{}'""".format(email))
...
def suggestion():
    ...
    curs.execute("""INSERT INTO  suggestion(email,message) VALUES('{}','{}')""".format(email,suggesMess))
    conn.commit()
